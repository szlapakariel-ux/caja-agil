  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 flex flex-col gap-4">
          ...
          <CardBody className="flex items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <div
                  className={`w-2.5 h-2.5 rounded-full ${data?.openSession ? "bg-green-500" : "bg-gray-300"}`}
                />
                <span className="font-semibold text-gray-900 dark:text-gray-100">
                  {data?.openSession
                    ? `Caja abierta — ${data.openSession.code}`
                    : "Sin caja abierta"}
                </span>
              </div>
              {data?.openSession && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 ml-4">
                  Apertura: {formatDateTime(data.openSession.openedAt)} ·{" "}
                  {data.openSession.openedBy.name} ·{" "}
                  Inicial: {formatCurrency(data.openSession.openingAmount)}
                </p>
              )}
              {!data?.openSession && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 ml-4">
            ...
            <div className="flex gap-2 flex-shrink-0">
              ...
              {data?.openSession && isAdmin && (
                <Link href="/closures">
                  <Button variant="secondary" size="sm">Cierre</Button>
                </Link>
              )}
            </div>
          </CardBody>
        ...
        {/* Alertas */}
        {(data?.summary?.pendingExpenses ?? 0) > 0 && isAdmin && (
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                {data?.summary?.pendingExpenses} egreso(s) pendiente(s) de revisión
              </p>
            </div>
            <Link href="/history">
        ...
        {(data?.pendingClosures?.length ?? 0) > 0 && isAdmin && (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-blue-800 dark:text-blue-200">
                Cierre(s) parcial(es) pendiente(s) de revisión
              </p>
            </div>
          </div>
        )}
          ...
          <CardBody>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-3">
                ...
                <p className="text-lg font-bold text-orange-700 dark:text-orange-300 mt-0.5">
                  {formatCurrency(summary?.ownerWithdrawals ?? 0)}
                </p>
              </div>
              <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-3">
                ...
                <p className="text-lg font-bold text-green-700 dark:text-green-300 mt-0.5">
                  {formatCurrency(summary?.ownerContributions ?? 0)}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                  {formatCurrency(summary?.expectedBalance ?? 0)}
                </span>
              </div>
        ...
        <div>
          ...
          {(data?.recentMovements?.length ?? 0) === 0 ? (
            <Card>
              <CardBody>
                <p className="text-center text-gray-400 py-4">No hay movimientos hoy</p>
              </CardBody>
            </Card>
          ) : (
            <div className="flex flex-col gap-2">
              {(data?.recentMovements as Parameters<typeof MovementCard>[0]["movement"][])?.map(
                (mov) => (
                  <MovementCard
